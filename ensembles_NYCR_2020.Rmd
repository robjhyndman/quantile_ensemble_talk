...
```{r comparison1, eval=TRUE, dependson='samples'}
train <- auscafe %>%
  filter(year(date) <= 2018)
fc <- train %>%
  model(
...
fc2 %>% autoplot(train %>% filter(year(date) >= 2017)) +
  autolayer(filter(fc2, .model=="Combination"), color='#DC3220') +
  autolayer(filter(fc2, .model=="Ensemble"), color='#E66100') 
```
...
\begin{block}{}
\begin{itemize}
\item Combinations involve averaging the distributions.
\item Ensembles involve mixing the distributions.
